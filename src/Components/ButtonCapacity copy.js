...
const ButtonCapacity = ({ player }) => {
    ...
    const combat = () => {
      if (currentPlayer && currentPlayer.pv > 0 && currentPlayer.mana > 0) {
        ...
        setActionMessage(`${currentPlayer.name} a hit le boss au tour d'avant`);
      } else {
    ...
    const handleSpecialAction = () => {
      if (currentPlayer && currentPlayer.pv > 0) {
        ...
        setActionMessage(`${currentPlayer.name} s'est soigné au tour d'avant`);
    ...
    const handleSpecialHit = () => {
      if (currentPlayer && currentPlayer.pv > 0 && currentPlayer.mana > 0) {
        ...
        dispatch(hitBack(player.id));
        dispatch(nextTurn()); // Ajoutez cette ligne
        setActionMessage(`${currentPlayer.name} a Megahit le boss au tour d'avant `);
      };
    };
    const handlequit = () => {
